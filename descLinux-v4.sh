#!/bin/sh
# descLinux-v4.sh — Detecta productos IBM instalados en un host Unix/Linux.
# Deriva de v3. Mejoras en v4:
#   - find con prune más conservador para compatibilidad con SOs viejos (SUSE 9, OL 5.x)
#   - Timeout con fallback mejorado via kill para entornos sin coreutils timeout
#   - Eliminado cat duplicado al final (OUTPUT_FILE ya se escribe via salida())
#   - Timestamp de ejecución agregado al reporte (paridad con script Windows)
...
PROG=${0##*/}
OUTPUT_FILE=./descLinux-v4.sh.txt
SEPARADOR='************************************************************'
...
probar_version_informix_componente() {
    ...
    if [ -n "$base_dir" ] && [ -x "$base_dir/bin/oninit" ]; then
        output=$(ejecutar_seguro 3 "$base_dir/bin/oninit" -V)
        version=$(printf '%s\n' "$output" | extraer_version_informix)
        if [ -n "$version" ]; then
...
TMPDIR=${TMPDIR:-/tmp}
HALLAZGOS=$TMPDIR/descLinux-v4.$$.hits
RESULTADOS=$TMPDIR/descLinux-v4.$$.results
...
# Uso de -path con -prune es POSIX; sin embargo, en find de SOs viejos
# (SUSE 9, OL 5.x, find de BusyBox o find de fileutils <4.2) la combinación
# \( -path A -o -path B \) -prune puede no excluir correctamente /proc y /sys,
# lo que causa que find intente leer entradas de kernel y se cuelgue.
# Estrategia defensiva:
#   1) Siempre incluir /proc y /sys como primeros candidatos al prune.
#   2) Usar -xdev si el entorno lo soporta para no cruzar filesystems virtuales.
#      (xdev es POSIX pero algunos find viejos lo ignoran silenciosamente — sin daño).
#   3) Limitar la profundidad de búsqueda en /proc explícitamente con -maxdepth
#      solo si find lo soporta (find GNU 4.1+); de lo contrario se confía en prune.

_find_supports_maxdepth=0
find /dev/null -maxdepth 0 >/dev/null 2>&1 && _find_supports_maxdepth=1
...
find / -xdev \( $prune_expr \) -prune -o \( $name_expr \) -type f -print 2>/dev/null | while IFS= read -r path; do
    sig=${path##*/}
    debe_conservarse "$sig" "$path" || continue
    printf '%s\t%s\n' "$sig" "$path" >> "$HALLAZGOS"
done

# Nota: los filesystems de red (NFS, CIFS) también quedan excluidos por -xdev,
# lo cual es el comportamiento deseado para un inventario local.
...
# Fix v4: se elimina "cat $OUTPUT_FILE" que existía en v3.
# emitir_reporte ya escribe al OUTPUT_FILE vía la función salida().
# El cat causaba que el contenido se emitiera dos veces en stdout
# al redirigir la salida del script.
